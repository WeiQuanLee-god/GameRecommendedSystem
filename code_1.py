...
#
# Notes:
# - Added Alternative Content-Based algorithm: TF-IDF + LSA (SVD) + Cosine kNN
# - You now have 4 options in sidebar: Content TF-IDF, Content LSA, CF, Hybrid
...
# ===== Content representation (TF-IDF + Cosine kNN)
@st.cache_data(show_spinner=False)
...
# ===== Alternative Content representation (TF-IDF + LSA + Cosine kNN)
@st.cache_data(show_spinner=False)
def build_content_matrix_lsa(df, n_components=100):
    text_corpus = df['category'] + ' ' + df['title'] + ' ' + df['description']
    vectorizer = TfidfVectorizer(lowercase=True, stop_words='english', ngram_range=(1,2), min_df=2)
    tfidf_matrix = vectorizer.fit_transform(text_corpus)
    ...
    return vectorizer, svd, latent_matrix

@st.cache_data(show_spinner=False)
def prepare_model(df: pd.DataFrame):
    vec, X = build_content_matrix(df)
    _, _, X_lsa = build_content_matrix_lsa(df, n_components=100)
...
# ===== Scoring functions
def recommend_content(df, X, seed_indices: List[int], top_k: int, mask: np.ndarray):
...
def recommend_content_lsa(df, X_lsa, seed_indices: List[int], top_k: int, mask: np.ndarray):
    base = np.zeros(len(df)) if len(seed_indices)==0 else cosine_similarity(X_lsa, X_lsa[seed_indices]).mean(axis=1).ravel()
    scores = base.copy()
    scores[~mask] = -1
    if len(seed_indices): scores[seed_indices] = -1
    order = np.argsort(scores)[::-1]
...
def recommend_collab_item_item(df, sim_mat: np.ndarray, seed_indices: List[int], top_k: int, mask: np.ndarray):
...
# ===== Evaluation helpers
def precision_recall_at_k(recommended: List[int], relevant: set, k: int):
    if k == 0:
        return 0.0, 0.0
    rec_k = recommended[:k]
...
def diversity_gini(indices: List[int], X):
    if len(indices) < 2:
        return 0.0
    vecs = X[indices]
...
try:
    ...
    vec, X, X_lsa, boosts = prepare_model(df)
...
algo = st.sidebar.selectbox("Choose approach to **show output**", [
    ...
    "Content-Based — TF-IDF + LSA (SVD) + Cosine kNN",
...
if algo.startswith("Content-Based — TF-IDF + Cosine"):
    top_idx, scores = recommend_content(df, X, seed_idx, int(k), mask_arr)
elif algo.startswith("Content-Based — TF-IDF + LSA"):
    top_idx, scores = recommend_content_lsa(df, X_lsa, seed_idx, int(k), mask_arr)
    ...
    for rank, i in enumerate(top_idx, start=1):
        ...
        with cols[3]:
            reason = {
                ...
                "Content-Based — TF-IDF + LSA (SVD) + Cosine kNN": "latent semantic similarity using reduced topics",
...
colA, colB, colC = st.columns(3)  # ✅ define columns first

with colA:
    st.metric("RMSE", "12.5")  # replace with real computation
with colB:
    st.metric("Precision@5", "0.78")
with colC:
    st.metric("Recall@5", "0.64")

st.info("These metrics are placeholders. Replace with actual evaluation results from your model testing.")
